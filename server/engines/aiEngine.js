 ...
 * 한자(CJK) 포함 여부 감지
 */
// 한글, 영어, 숫자, 기본 문장부호 외 모든 문자 감지 (화이트리스트 방식)
function containsForeignChars(text) {
  return /[^\uAC00-\uD7A3\u1100-\u11FF\u3130-\u318Fa-zA-Z0-9\s.,!?~\-_'"()+%·…\u2019\u201C\u201D😊🙏🏠]/.test(text);
}

// 한글, 영어, 숫자, 기본 문장부호만 남기고 나머지 제거
function stripForeignChars(text) {
  return text
    .replace(/[^\uAC00-\uD7A3\u1100-\u11FF\u3130-\u318Fa-zA-Z0-9\s.,!?~\-_'"()+%·…\u2019\u201C\u201D😊🙏🏠]/g, '')
    .replace(/\s{2,}/g, ' ')
    .trim();
}
...
- 한자(漢字), 일본어 히라가나(あいう), 가타카나(アイウ), 중국어 간체·번체는 절대 사용하지 마세요.
- 예를 들어 "疲勞", "休息", "運動", "です", "ます", "カフェ" 같은 표현 대신 반드시 "피로", "휴식", "운동", "카페"처럼 한글로 쓰세요.`;
...
반드시 아래 JSON 형식으로만 응답하세요 (다른 텍스트 없이):
{
  ...
  // 한글/영어 외 문자가 포함된 경우 서버 측에서 제거 (안전망)
  if (containsForeignChars(parsed.reason)) {
    console.warn('[aiEngine] reason에 외국어 감지 — 자동 제거');
    parsed.reason = stripForeignChars(parsed.reason);
  }
  if (containsForeignChars(parsed.recommendation)) {
    console.warn('[aiEngine] recommendation에 외국어 감지 — 자동 제거');
    parsed.recommendation = stripForeignChars(parsed.recommendation);
  }
